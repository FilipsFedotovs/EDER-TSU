#This simple script prepares 2-segment track seeds for the initial CNN gluing
...
output_file_location=EOS_DIR+'/EDER-TSU/Data/TRAIN_SET/M2_M2_RawTracks_'+Set+'_'+str(Subset)+'.csv'
output_result_location=EOS_DIR+'/EDER-TSU/Data/TRAIN_SET/M2_M2_RawTracks_'+Set+'_'+str(Subset)+'_RES.csv'
...
data_e=pd.merge(data, data_header, how="inner", left_on=["FEDRA_Seg_ID",'MC_Mother_Track_ID',"z"], right_on=["FEDRA_Seg_ID",'MC_Mother_Track_ID',"e_z"]) #Shrinking the Track data so just a star hit for each track is present.
