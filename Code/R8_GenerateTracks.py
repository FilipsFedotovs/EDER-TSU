...
if Mode=='R':
   ...
   if UserAnswer=='Y':
      ...
      for j in range(0,len(data)):
          ...
          ScriptName = AFS_DIR + '/Code/Utilities/R8_GenerateTracks_Sub.py '
          UF.SubmitJobs2Condor([OptionHeader, OptionLine, SHName, SUBName, MSGName, ScriptName, int(data[j][2]), 'EDER-TSU-R8', False,False])
...
if Mode=='C':
   ...
   for j in range(0,len(data)):
       for sj in range(0,int(data[j][2])):
           ...
           SHName = AFS_DIR + '/HTCondor/SH/SH_R8_' + str(j) + '_' + str(sj) + '.sh'
           SUBName = AFS_DIR + '/HTCondor/SUB/SUB_R8_' + str(j) + '_' + str(sj) + '.sub'
           MSGName = AFS_DIR + '/HTCondor/MSG/MSG_R8_' + str(j) + '_' + str(sj)
           ScriptName = AFS_DIR + '/Code/Utilities/R8_GenerateTracks_Sub.py '
           job_details = [OptionHeader, OptionLine, SHName, SUBName, MSGName, ScriptName, 1, 'EDER-TSU-R8', False,False]
           output_file_location=EOS_DIR+'/EDER-TSU/Data/REC_SET/R8_R8_RawTracks_'+str(j)+'_'+str(sj)+'.csv'
           output_result_location=EOS_DIR+'/EDER-TSU/Data/REC_SET/R8_R8_RawTracks_'+str(j)+'_'+str(sj)+'_RES.csv'
   ...
   if len(bad_pop)>0:
     ...
     if UserAnswer=='R':
        ...
        for sj in range(0,int(data[j][2])):
           ...
           if os.path.isfile(output_file_location):
            ...
            for f in range(0,fractions):
             new_output_file_location=EOS_DIR+'/EDER-TSU/Data/REC_SET/R8_R9_RawTracks_'+str(j)+'_'+str(sj)+'_'+str(f)+'.csv'
             ...
             for j in range(0,len(data)):
                for sj in range(0,int(data[j][2])):
                    for f in range(0,1000):
                      new_input_file_location=EOS_DIR+'/EDER-TSU/Data/REC_SET/R8_R9_RawTracks_'+str(j)+'_'+str(sj)+'_'+str(f)+'.csv'
       ...
       exit()
       UF.RecCleanUp(AFS_DIR, EOS_DIR, 'R8', ['R8_R8'], "SoftUsed == \"EDER-TSU-R8\"")
       print(bcolors.HEADER+"########################################################################################################"+bcolors.ENDC)
