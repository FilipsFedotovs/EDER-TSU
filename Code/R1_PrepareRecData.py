...
parser.add_argument('--Quadrant',help='Would you like to select only one quadrant?', default='ALL')
########################################     Main body functions    #########################################
...
if Quadrant!='ALL':
     print(UF.TimeStamp(),'Keeping only the quadrant ', Quadrant)
     data=data.drop(data.index[(data[PM.FEDRA_Track_QUADRANT] != Quadrant)])
     final_rows=len(data.axes[0])
...
if args.Log=='Y':
   try:
      ...
      UF.LogOperations(EOS_DIR+'/EDER-TSU/Data/REC_SET/R_LOG.csv', 'StartLog', [['Step_No','Step_Desc','Fake_Seeds','Truth_Seeds','Precision','Recall'],[1,'Initial Sampling',rec_no,eval_rec,eval_rec/(rec_no+eval_rec),1.0]])
